...
- **Override manuali:** `PROJECT_REPO_OVERRIDES` definisce solo dati tecnici o di presentazione che non conviene prendere da GitHub: titolo leggibile, lista linguaggi, linguaggio principale, `hideDemoButton`, `disablePreview` e `demoUrl` opzionale per casi speciali.
  - `Sito_Web_Portfolio`: visibile, solo pulsante Repository, preview disabilitata per evitare di aprire il sito dentro se stesso.
  - `Login_Registration_Form_Deploy`: visibile, linguaggi `HTML / CSS / JavaScript / Python`, linguaggio principale `Python`; la demo viene ricavata automaticamente da `repo.homepage` o da GitHub Pages (`repo.has_pages`).
  - `LinkTree`: visibile, linguaggi `HTML / CSS`, linguaggio principale `HTML`; conserva `demoUrl` finche' il campo Website/homepage della repository GitHub non punta alla demo Netlify.
...
- **Card:** l'anteprima mostra titolo e lista linguaggi (`languages`); nel corpo viene mostrata la descrizione da `repo.description` di GitHub, con fallback i18n `project_no_description`, e `Linguaggio piu utilizzato: ...` usando `mainLanguage`; la data mostrata e' `created_at` della repo GitHub, formattata come mese/anno con prima lettera maiuscola.
...
- **i18n:** le stringhe della sezione Progetti sono presenti in tutte le lingue supportate, incluse le label accessibili dei pulsanti freccia.
...
Codex ha aggiornato la sezione Progetti: le card restano generate automaticamente dalle repository pubbliche GitHub di `StefaDale`, ordinate per data di creazione, con slider orizzontale e frecce che scorrono una repository alla volta. Le descrizioni arrivano da `repo.description` su GitHub; `PROJECT_REPO_OVERRIDES` resta limitato a dati tecnici/di presentazione come titolo leggibile, linguaggi, preview disabilitata e `demoUrl` opzionale. Le demo live vengono risolte in ordine da `override.demoUrl`, `repo.homepage` e GitHub Pages (`repo.has_pages`). Le stringhe i18n della sezione Progetti sono presenti in tutte le lingue supportate e validate. Il viewer inline del CV resta disattivato in modo reversibile: `.cv-wrapper`, gli script PDF.js, `renderCustomCV()` e le relative chiamate sono commentati con istruzioni di ripristino.
